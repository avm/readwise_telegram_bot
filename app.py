...
async def send_to_readwise(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        if update.message.caption_html is None
...
async def prepare_readwise(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text="Sending data to Readwise..."
    )
    return FORWARD
...
async def send_to_reader(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    WISE.save(
        url=telegram_link,
        html=text,
        title=str(update.message.forward_from_chat.username)
        + " "
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text="Working with Reader API..."
    )
...
if __name__ == "__main__":
    ...
    conv_handler_reader = ConversationHandler(
        ...
        states={
            FORWARD: [
                MessageHandler(
                    (filters.TEXT | filters.ATTACHMENT | filters.PHOTO)
                    & ~filters.COMMAND,
                    send_to_readwise,
                )
            ],
    ...
    application.add_handler(
        MessageHandler(
            (filters.TEXT | filters.ATTACHMENT | filters.PHOTO) & ~filters.COMMAND,
            send_to_reader,
        )
    )
